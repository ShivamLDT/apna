...
const EndpointInfoPopup = ({ loading, selectedEndpoint, onClose }) => {
    if (loading) {
        return (
                ...
                <div className="bg-white rounded-lg max-w-md w-full mx-4">
                    <div className="p-6">
                        <div className="flex items-center justify-center py-8">
                            ...
                            <span className="ml-2 text-gray-600">Loading Endpoint data...</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const { system, os, memory, disk, network, processor, ip_addresses, mac_addresses } = selectedEndpoint.data;
    ...
    return (
        <div className="Endpoint-popup-overlay">
            <div className="Endpoint-popup resizable-popup">
                <div className="Endpoint-popup-drag-handle">
                    <button className="close-btnnn" onClick={onClose}>X</button>
                    <h2 className="Endpoint-header">Endpoint: {system?.name}</h2>
                </div>

                <div className="EndpointInfo-section">
                    <div className="Endpoint-box device-info">
                        ...
                        <p><strong>Server Version:</strong> {selectedEndpoint?.server_version}</p>
                        <p><strong>IP:</strong> {ip_addresses}</p>
                        <p><strong>MAC:</strong> {mac_addresses}</p>
                        {/* <p><strong>Activation Date:</strong> {selectedEndpoint?.server_activation_date}</p> */}
                    ...
                    <div className="Endpoint-box Endpoint-chart-box Endpoint-ntwk-bar">
                        <h3>Network Info</h3>
                        ...
                        <span className='Endpoint-nterk-infocc'>
                            <label>Received</label>
                            <p>{formattedReceived}</p>
                        </span>
                        <span className='Endpoint-nterk-infocc1'>
                            <label>Data Sent</label>
                            <p>{formattedSent}</p>
                        </span>
                    </div>

                    <div className="Endpoint-box Endpoint-chart-box Endpoint-procs-bar">
                        <h3>Processor Usage</h3>
                        <Doughnut data={processorChart} />
                        <span className='Endpoint-procs-infocc'>
                            <label>Usage</label>
                            <p>{processor.usage}%</p>
                        </span>
                        <span className='Endpoint-procs-infocc11'>
                            <label>Count</label>
                            <p>{processor.count}</p>
                        </span>
                        <span className='Endpoint-procs-infocc12'>
                            <label>Cores</label>
                            <p>{processor.physical}</p>
                        </span>
                    </div>

                    <div className="Endpoint-box Endpoint-chart-box Endpoint-disk-bar">
                        <h3>Disk Storage</h3>
                        <Bar data={diskBarChart} />
                    </div>

                    <div className="Endpoint-box Endpoint-chart-box Endpoint-mem-bar">
                        <h3>Memory Info</h3>
                        <Doughnut data={memoryChart} />
                        <span className='Endpoint-memo-infocc'>
                            <label>Total</label>
                            <p>{memory.total}</p>
                        </span>
                        <span className='Endpoint-memo-infocc11'>
                            <label>Available</label>
                            <p>{memory.available}</p>
                        </span>
                        <span className='Endpoint-memo-infocc12'>
                            <label>Used</label>
                            <p>{memory.used}</p>
                        </span>
                    </div>
...
export default EndpointInfoPopup;
