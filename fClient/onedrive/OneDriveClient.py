...
from fClient.fingerprint import get_encryption_key_storage
...
class OneDriveClient:
    ...
    def upload_file(self, local_path, remote_path):
        ...
        try:
            ...
            if isinstance(local_path, dict):
                file_data=io.BytesIO(local_path["file"][1])
                local_path=local_path["file"][0]
            else:
    ...
    @staticmethod
    def send_enscdata(file_path='OneDrive_credentials.enc',encrypt_key=None):
        # from flask import send_file
        agent_path=file_path.replace(".enc",f"_{encrypt_key}.enc")
        json_data={}
        try:
            key = get_encryption_key_storage(file_path)        
            cipher = Fernet(key)
            with open(file_path, "r") as file:
                json_data = file.read()
                json_data =cipher.decrypt(json_data.encode()).decode()
                json_data= encrypt_data(json_data,encrypt_key)
