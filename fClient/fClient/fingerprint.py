...
LOG_FILE = "every_logs/client_fpt.log"
...
'''
followin function are commented to discontinue WMIC
def get_hdd_serials():
    hdd_serials = []
    if platform.system() == "Windows":
        ...
        hdd_serials = [serial.strip() for serial in output if serial.strip()]
...
def get_cpu_ids():
    try:    
        ...
        if platform.system() == "Windows":
            ...
            cpu_ids = [cpu_id.strip() for cpu_id in output if cpu_id.strip()]
        elif platform.system() == "Linux":
...
def get_motherboard_serial():
    ...
    try:    
        ...
        if platform.system() == "Windows":
            ...
            serial_number = output[0].strip() if output else None
        elif platform.system() == "Linux":
...
def get_hdd_serials() -> List[str]:
    ...
    try:
        if platform.system() == "Windows":
            ...
            hdd_serials = hdd #[drive.SerialNumber.strip() for drive in drives if drive.SerialNumber]
        elif platform.system() == "Linux":
...
def get_cpu_ids() -> List[str]:
    ...
    try:
        if platform.system() == "Windows":
            ...
            cpu_ids = cpu # [cpu.ProcessorId.strip() for cpu in cpus if cpu.ProcessorId]
        elif platform.system() == "Linux":
...
def get_motherboard_serial() -> Optional[str]:
    try:
        if platform.system() == "Windows":
            ...
            return board # [0].SerialNumber.strip() if boards else None
        elif platform.system() == "Linux":
...
def getCode():
    ...
    ###################################################################################################################################### 
        ...
        us = f"ku{cpu_ids}ld-ee{motherboard_serial}pr-aa{bios_serial}js-ha{bios_serial}rm-a{cpu_ids}"
        logger.info(f"For  testing check difference client abc {us}")
        uh = hashlib.sha256(us.encode("utf-8")).hexdigest()
        return uh
...
def getRequestKey():
    ...
    json_data = {
        "email": "",
        "IPname": getCodeHost(),
        "application": "Apna-Backup-Endpoint",
...
def get_miltiprocessing_cpu_count():
