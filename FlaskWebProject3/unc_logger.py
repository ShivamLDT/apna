"""
Unified UNC/SMB Logging Module for Server-Side Operations.
Provides consistent logging for all UNC/SMB operations including:
- Connection attempts and results
- Path normalization
- Browse operations (browseUNC endpoint)
...
unc_logger = logging.getLogger("UNC_SMB_SERVER")
...
def log_browse_unc_request(host, path, method, client_ip=None):
    """Log browseUNC API request."""
    msg = f"BROWSE_UNC_REQUEST | host={host} | path={path} | method={method}"
    if client_ip:
