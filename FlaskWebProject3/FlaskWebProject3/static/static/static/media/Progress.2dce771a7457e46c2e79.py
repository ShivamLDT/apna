...
# # Example data with multiple jobs per agent at different times
# backup_jobs = [
#     {"name": f"Backup Job {i}", "scheduled_time": f"10:{i:02}:00", "agent": "AgentA", 
#      "progress_number": random.randint(0, 100), "restore_flag": False}
#     for i in range(1, 6)
# ] + [
#     {"name": f"Backup Job {i+5}", "scheduled_time": f"12:{i:02}:00", "agent": "AgentA", 
#      "progress_number": random.randint(0, 100), "restore_flag": False}
#     for i in range(1, 4)
# ] + [
#     {"name": f"Backup Job {i+8}", "scheduled_time": f"11:{i:02}:00", "agent": "AgentA", 
#      "progress_number": random.randint(0, 100), "restore_flag": False}
#     for i in range(1, 7)
# ] + [
#     {"name": f"Backup Job {i+14}", "scheduled_time": f"15:{i:02}:00", "agent": "AgentB", 
#      "progress_number": random.randint(0, 100), "restore_flag": False}
#     for i in range(1, 6)
# ] + [
#     {"name": f"Backup Job {i+19}", "scheduled_time": f"19:{i:02}:00", "agent": "AgentB", 
#      "progress_number": random.randint(0, 100), "restore_flag": False}
#     for i in range(1, 4)
# ] + [
#     {"name": f"Backup Job {i+22}", "scheduled_time": f"18:{i:02}:00", "agent": "AgentB", 
#      "progress_number": random.randint(0, 100), "restore_flag": False}
#     for i in range(1, 7)
# ]
...
#     for job in backup_jobs:
#         if restore_flag:
#             # Decrease progress from 100 to 0
#             previous_jobs = previous_backup_jobs[job['agent']]
#             previous_job = previous_jobs.get(job['name'])
...
#     restore_flag = False
#     while True:
#         # Update restore_flag every 5 seconds
#         restore_flag = random.choice([True, False])

#         updated_jobs = generate_backup_data(restore_flag)
#         if updated_jobs:  # Emit only if there are updated jobs
#             socketio.emit('backup_data', {"backup_jobs": updated_jobs, "restore_flag": restore_flag})
#             print("Sent backup data:", {"backup_jobs": updated_jobs, "restore_flag": restore_flag})  # Debug print to verify data
...
def generate_backup_data(restore_flag):
    ...
    for job in backup_jobs:
        if restore_flag:
            # Decrease progress from 100 to 0
            job_name = f"Restored Job {job['name'].split()[-1]}"  # Change job name for restore
            previous_jobs = previous_backup_jobs[job['agent']]
            previous_job = previous_jobs.get(job['name'])
            if previous_job and previous_job['progress_number'] > 0:
                new_progress = max(0, previous_job['progress_number'] - random.randint(1, 5))
                if new_progress != previous_job['progress_number']:
                    updated_job = job.copy()
                    updated_job['name'] = job_name
                    updated_job['progress_number'] = new_progress
                    previous_backup_jobs[job['agent']][job['name']] = updated_job
                    updated_jobs.append(updated_job)
        else:
            # Simulate progress updates at random intervals
            job_name = f"Active Job {job['name'].split()[-1]}"  # Change job name for active state
            if random.random() < 0.1:  # 10% chance to update a job
                previous_jobs = previous_backup_jobs[job['agent']]
                previous_job = previous_jobs.get(job['name'])
                if previous_job:
...
#     while True:
#         for job in backup_jobs:
#             # Randomly decide if this job is in restore mode
#             restore_flag = random.choice([True, False])
#             previous_jobs = previous_backup_jobs[job['agent']]
#             previous_job = previous_jobs.get(job['name'])

#             # If the job is being restored
#             if restore_flag:
#                 if previous_job['progress_number'] == 0:
#                     new_progress = 100  # Reset progress to 100 at the start of restoration
#                 else:
#                     new_progress = max(0, previous_job['progress_number'] - 1)  # Decrease progress by 1
#             else:
#                 # Regular job update
#                 new_progress = min(100, previous_job['progress_number'] + random.randint(1, 5))

#             # Update the job only if the progress has changed
#             if new_progress != previous_job['progress_number']:
#                 updated_job = job.copy()  # Keep the original job structure
#                 updated_job['progress_number'] = new_progress
#                 updated_job['restore_flag'] = restore_flag  # Include the restore_flag in the job data
#                 previous_backup_jobs[job['agent']][job['name']] = updated_job

#                 # Emit the job data with the existing structure
#                 socketio.emit('backup_data', {"backup_jobs": [updated_job]})  # Emit the job
#                 print("Sent backup data:", {"backup_jobs": [updated_job]})  # Debug print

#             time.sleep(10)  # Wait for 10 seconds before sending the next job's data
...
def send_progress_updates():
    while True:
        for job in backup_jobs:
            ...
            # Set restore_flag to True for all jobs
            job['restore_flag'] = True  # Send True in restore_flag

            # Example condition to trigger restore: check if the job is complete
            if previous_job['progress_number'] == 100 and not previous_job['restore_flag']:
                print(f"Triggered restore for {job['name']}.")

            if job['restore_flag']:  # Job A: Decrease from 100 to 0
                if previous_job['progress_number'] == 0:
                    new_progress = 100  # Reset to start restore
                else:
                    new_progress = max(0, previous_job['progress_number'] - 1)

            else:  # Job B: Increase from 0 to 100
                if previous_job['progress_number'] == 100:
                    new_progress = 0
                else:
                    new_progress = min(100, previous_job['progress_number'] + 1)
            ...
            if new_progress != previous_job['progress_number']:
                ...
                # Emit the job data with the existing structure
                socketio.emit('backup_data', {"backup_jobs": [updated_job]})
                print("Sent backup data:", {"backup_jobs": [updated_job]})

        time.sleep(10)  # Wait for 10 seconds before starting the next round
